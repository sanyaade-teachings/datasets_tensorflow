...
def test_convert_value_raises(hf_value, feature):
  with pytest.raises(
      ValueError, match='Conversion of value.+ is not supported.'
  ):
    huggingface_utils._convert_hf_value(hf_value, feature)
...
def test_convert_value(hf_value, feature, expected_value):
  assert (
      huggingface_utils._convert_hf_value(hf_value, feature) == expected_value
  )
